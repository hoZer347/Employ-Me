import "./style.css";
import { resolveVanguardsSource, vanguardsGameUrl } from "./vanguards";

const root = document.querySelector<HTMLDivElement>("#vanguards")!;
  ...
  <div class="flex min-h-screen flex-col bg-slate-950 text-slate-100">
    <header class="flex items-center justify-between gap-4 px-4 py-3">
      ...
      <span class="text-sm font-medium">
        Vanguards <span id="v-version" class="text-slate-500"></span>
      </span>
    </header>
...
async function boot() {
  const source = await resolveVanguardsSource();
  const url = vanguardsGameUrl(source);
  versionLabel.textContent = `v${source.version}`;

  const iframe = document.createElement("iframe");
  iframe.src = url;
  iframe.title = "Vanguards";
